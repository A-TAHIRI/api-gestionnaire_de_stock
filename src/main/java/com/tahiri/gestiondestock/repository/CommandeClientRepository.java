...
/*********************************************************** nombre de commande **********************************************************************************/
     ...
     * @return
     */
    @Query("SELECT c FROM CommandeClient c WHERE YEAR(c.createDate) = YEAR(CURRENT_DATE) AND MONTH(c.createDate) = MONTH(CURRENT_DATE )   ORDER BY c.totalPrix DESC LIMIT 5")
    List<CommandeClient> findCmdCltByMonthByOrderByTotalPrixDesc();
